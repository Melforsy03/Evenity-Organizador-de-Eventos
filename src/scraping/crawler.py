"""
Módulo de scraping de eventos desde APIs externas (crawler.py)

Define la clase `EventScraper`, responsable de extraer eventos desde:
- PredictHQ
- SeatGeek
- Ticketmaster

El scraping se realiza en paralelo con `ThreadPoolExecutor`, y los eventos se guardan como archivos JSON
en la carpeta `eventos_completos`.

Funciones:
- `run_all_scrapers()` → ejecuta los 3 scrapers
- `scrape_predicthq_events()`, `scrape_seatgeek_events()`, `scrape_ticketmaster_events()` → descarga por fuente
- `process_<fuente>_event()` → normaliza eventos antes de guardarlos
- `save_event()` → guarda el JSON en disco
"""
